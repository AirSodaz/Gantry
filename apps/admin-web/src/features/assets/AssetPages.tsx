...
export function AssetDetailPage({ kind }: { kind: AssetKind }) {
  ...
  const enablePlugin = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['admin-asset', kind, assetId] });
    },
  });
  ...
  return <section className="admin-page">
    ...
    <div className="admin-detail-grid">
      ...
      { 'workspaces' in item ? <section className="admin-detail-block"><h2>Workspace enablement</h2>{item.workspaces.length === 0 ? <p className="admin-muted">No workspace enablements.</p> : <ul className="admin-detail-list">{item.workspaces.map((workspace) => <li key={workspace.id}><div className="admin-overview-highlight"><strong>{workspace.display_name}</strong><Button size="sm" variant="quiet" disabled={disablePlugin.isPending || enablePlugin.isPending} title={`Disable in ${workspace.display_name}`} onClick={() => disablePlugin.mutate(workspace.id)}><Unplug size={14} /> Disable</Button></div><span>{workspace.id}</span></li>)}</ul>}<div className="admin-form-row"><select className="ds-input" value={enableWorkspaceID} onChange={(event) => setEnableWorkspaceID(event.target.value)} disabled={workspaces.isLoading || enablePlugin.isPending}><option value="">Choose a workspace to enable</option>{(workspaces.data?.items ?? []).filter((workspace) => !item.workspaces.some((enabled) => enabled.id === workspace.id)).map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.display_name}</option>)}</select><Button size="sm" variant="secondary" disabled={!enableWorkspaceID || enablePlugin.isPending} onClick={() => enablePlugin.mutate()}><Plus size={14} /> Enable</Button></div>{disablePlugin.error ? <p className="admin-error" role="alert">{disablePlugin.error.message}</p> : null}{enablePlugin.error ? <p className="admin-error" role="alert">{enablePlugin.error.message}</p> : null}</section> : null }
...
function AssetCatalog({ kind }: { kind: AssetKind }) {
  ...
  const mutation = useMutation<AssetItem, Error, void>({
    mutationFn: async () => {
      ...
      if (kind === 'plugins') {
        return api.registerPlugin({ slug: form.slug ?? '', display_name: form.display_name ?? '', description: form.description ?? '', version: form.version ?? '', content_digest: form.content_digest ?? '', manifest_json: parseObject(form.manifest_json, 'Plugin manifest') });
      }
      let schema: Record<string, unknown> = {};
      if (form.schema_json?.trim()) {
        const parsed = JSON.parse(form.schema_json);
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('Schema JSON must be an object.');
        schema = parsed as Record<string, unknown>;
      }
      return api.registerTool({ server_name: form.server_name ?? '', server_type: (form.server_type ?? 'builtin') as Tool['server_type'], endpoint_ref: form.endpoint_ref ?? '', fully_qualified_name: form.fully_qualified_name ?? '', version: form.version ?? '', effect: (form.effect ?? 'read') as Tool['effect'], idempotency: (form.idempotency ?? 'read_only') as Tool['idempotency'], content_digest: form.content_digest ?? '', schema_json: schema });
    },
