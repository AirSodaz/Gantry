...
function AssetCatalog({ kind }: { kind: AssetKind }) {
  ...
  const mutation = useMutation<AssetItem, Error, void>({
    mutationFn: async () => {
      ...
      if (kind === 'plugins') {
        const plugin = await api.registerPlugin({ slug: form.slug ?? '', display_name: form.display_name ?? '', description: form.description ?? '', version: form.version ?? '', content_digest: form.content_digest ?? '', manifest_json: parseObject(form.manifest_json, 'Plugin manifest') });
