...
function CaseSection({ suiteId, input, onInputChange }: { suiteId: string; input: string; onInputChange: (value: string) => void }) {
  const api = useAdminApi();
  ...
  return <section className="evaluation-section"><SectionHeading eyebrow="Working copy" title="Cases" icon={<ClipboardCheck size={18} />} />{cases.data?.items.map((item) => <div className="evaluation-row" key={item.id}><strong>{item.id}</strong><span>ETag {item.etag} · fixture manifest pinned</span></div>)}<div className="evaluation-command"><label className="admin-field"><span className="admin-field-label">Case input</span><input className="ds-input" value={input} onChange={(event) => onInputChange(event.target.value)} /></label><Button size="sm" onClick={() => create.mutate()} isLoading={create.isPending}><FilePlus2 size={15} /> Add case</Button></div>{create.error ? <p className="admin-error" role="alert">Case input must be a JSON object.</p> : null}</section>;
}
