...
export function AgentOverviewPage() {
  ...
  return <section className="admin-page">
    <header className="admin-detail-heading"><div><div className="admin-detail-title"><Bot size={22} /><h1>{agent.display_name}</h1><StatusMark status={agent.lifecycle_status} /></div><p>{agent.description}</p></div><div className="admin-header-actions"><Link className="ds-button ds-button-quiet" to={`/agents/${agent.id}/versions`}><GitBranch size={16} /> Versions</Link><Link className="ds-button ds-button-primary" to={`/agents/${agent.id}/design`}><ArrowRight size={16} /> Edit Main Draft</Link></div></header>
