...
import { Activity, ArrowDownToLine, ArrowLeft, ArrowRight, FileSearch, ShieldCheck } from 'lucide-react';
...
export function AuditPage() {
  ...
  const exportOptions = { workspaceId: workspace, resourceType, resourceId, eventType, actorId, outcome, risk, correlationId, runId, revisionHash };
  return <section className="admin-page">
    <header className="admin-page-heading"><div><h1>Audit</h1><p>Immutable, attributable configuration and runtime evidence.</p></div><AuditExportPanel options={exportOptions} /></header>
...
export function AuditEventDetailPage() {
  ...
  return <section className="admin-page"><Link className="admin-back-link" to="/audit"><ArrowLeft size={15} /> Audit</Link><header className="admin-detail-heading"><div><div className="admin-detail-title"><ShieldCheck size={22} /><h1>Audit event {event.id}</h1></div><p>{event.event_type} · {formatTime(event.created_at)}</p></div><AuditExportPanel options={{ resourceType: event.resource_type, resourceId: event.resource_id }} /></header>
...
function AuditExportPanel({ options }: { options: { workspaceId?: string; resourceType?: string; resourceId?: string; actorId?: string; eventType?: string; outcome?: string; risk?: string; correlationId?: string; runId?: string; revisionHash?: string } }) {
  ...
  const create = useMutation({ mutationFn: () => api.createAuditExport(options), onSuccess: (item) => setExportId(item.id) });
  const status = useQuery({
    queryKey: ['admin-audit-export', exportId],
    queryFn: () => api.getAuditExport(exportId),
    enabled: exportId !== '',
    refetchInterval: (query) => ['requested', 'processing'].includes(query.state.data?.state ?? '') ? 1000 : false,
  });
  const download = useMutation({ mutationFn: () => api.downloadAuditExport(exportId) });
  const item = status.data ?? create.data;
  return <div className="admin-export-panel"><Button size="sm" variant="secondary" isLoading={create.isPending} onClick={() => create.mutate()}><ArrowDownToLine size={15} /> Export evidence</Button>{item ? <span className="admin-export-status">{item.state}{item.package_digest ? ` · ${shortHash(item.package_digest)}` : ''}</span> : null}{item?.state === 'ready' ? <Button size="sm" variant="quiet" isLoading={download.isPending} onClick={() => download.mutate()}><ArrowDownToLine size={15} /> Prepare download</Button> : null}{download.data ? <a className="admin-inline-link" href={download.data.url} target="_blank" rel="noreferrer">Download package</a> : null}{create.error || status.error || download.error ? <span className="admin-error" role="alert">Audit export is unavailable for this account or scope.</span> : null}</div>;
}
