import type { AdminAuditEvent, AdminAuditEventDetail, AdminAuditExport, AdminAuditExportDownload, AdminOverview, AdminRun, AdminRunDetail, Agent, AgentDeployment, AgentLifecycleOverview, AgentRevision, AgentRevisionReview, AssetUsage, CreateAgentInput, CreatePolicyInput, EvaluationCase, EvaluationGate, EvaluationRegressionList, EvaluationRun, EvaluationSuite, EvaluationSuiteVersion, NamedAgentDraft, Plugin, PluginDetail, Policy, PolicyBinding, PolicyDraft, PolicySimulation, PolicyVersion, Skill, Tool, Workspace, Integration, IntegrationClient, AgentPublication, WebhookEndpoint, WebhookDelivery, ModelProvider, ProviderRoute, RunnerPool, Runner, CredentialReference, DataClassification, LimitPolicy, EnvironmentProfile, PlatformSettingsProjection, SettingsValidation } from './types';
...
export class AdminApi {
  ...
  listLimitPolicies(workspaceId?: string) { return this.request<{ items: LimitPolicy[] }>(`/platform/limit-policies${workspaceId ? `?workspace_id=${encodeURIComponent(workspaceId)}` : ''}`); }
  upsertLimitPolicy(id: string, etag: string, input: { workspace_id?: string; concurrency: number; duration_seconds: number; output_bytes: number; artifact_bytes: number; budget: Record<string, unknown> }) { return this.request<LimitPolicy>(`/platform/limit-policies/${encodeURIComponent(id)}`, { method: 'PUT', headers: { 'If-Match': `"${etag.replace(/^"|"$/g, '')}"` }, body: JSON.stringify(input) }); }
  listEnvironmentProfiles(workspaceId?: string) { return this.request<{ items: EnvironmentProfile[] }>(`/platform/environment-profiles${workspaceId ? `?workspace_id=${encodeURIComponent(workspaceId)}` : ''}`); }
  upsertEnvironmentProfile(id: string, etag: string, input: { workspace_id?: string; name: EnvironmentProfile['name']; publication_posture: EnvironmentProfile['publication_posture']; state: EnvironmentProfile['state']; data_classification_id?: string; allowed_target_controls: Record<string, unknown> }) { return this.request<EnvironmentProfile>(`/platform/environment-profiles/${encodeURIComponent(id)}`, { method: 'PUT', headers: { 'If-Match': `"${etag.replace(/^"|"$/g, '')}"` }, body: JSON.stringify(input) }); }
  getPlatformSettings(scope: 'organization' | 'workspace', workspaceId?: string) { const query = new URLSearchParams({ scope }); if (workspaceId) query.set('workspace_id', workspaceId); return this.request<PlatformSettingsProjection>(`/platform/settings?${query.toString()}`); }
  validatePlatformSettings(input: { workspace_id?: string; values: Record<string, unknown> }) { return this.request<SettingsValidation>('/platform/settings:validate', { method: 'POST', body: JSON.stringify(input) }); }
  applyPlatformSettings(etag: string, input: { workspace_id?: string; values: Record<string, unknown> }, idempotencyKey: string) { return this.request<PlatformSettingsProjection>('/platform/settings:apply', { method: 'POST', headers: { 'If-Match': `"${etag.replace(/^"|"$/g, '')}"`, 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }); }
