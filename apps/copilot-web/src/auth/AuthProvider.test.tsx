...
function Probe() {
  ...
  return (
    <div>
      <span data-testid="state">{auth.isLoading ? 'loading' : auth.user ? 'signed-in' : 'signed-out'}</span>
...
const user = { profile: { sub: 'user-1', preferred_username: 'demo' }, access_token: 'token-1' } as unknown as User;
