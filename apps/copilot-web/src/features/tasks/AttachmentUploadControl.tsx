...
import { CircleCheck, LoaderCircle, Paperclip, ShieldAlert, X } from 'lucide-react';
...
export function AttachmentUploadControl({ disabled = false, onChange }: {
  disabled?: boolean;
  onChange: (state: AttachmentUploadState) => void;
}) {
  const api = useCopilotApi();
  const [attachments, setAttachments] = useState<PendingAttachment[]>([]);
  const attachmentIDs = useMemo(() => attachments.flatMap((item) => item.state === 'available' && item.attachment ? [item.attachment.id] : []), [attachments]);
  const hasPending = attachments.some((item) => !['available', 'error'].includes(item.state));

  useEffect(() => onChange({ attachmentIDs, hasPending }), [attachmentIDs, hasPending, onChange]);
  ...
  return <>
      ...
      <ul className="attachment-list" aria-label="Selected attachments">
        {attachments.map((attachment) => (
          <li key={attachment.localId} className={`attachment-item attachment-${attachment.state}`}>
            ...
            <button type="button" className="ds-icon-button ds-icon-button-sm attachment-remove" onClick={() => setAttachments((current) => current.filter((item) => item.localId !== attachment.localId))} aria-label={`Remove ${attachment.filename}`} title={`Remove ${attachment.filename}`} disabled={disabled}><X size={14} /></button>
          </li>
        ))}
      </ul>
    ) : null}
    <label className="attachment-add" title="Add attachment">
      ...
      <input type="file" multiple disabled={disabled} onChange={(event) => { void selectAttachments(event.target.files); event.currentTarget.value = ''; }} />
    </label>
  </>;
}
