...
describe('getSubmissionKey', () => {
  it('reuses a key for the same request after a failed network attempt', () => {
    ...
    const first = getSubmissionKey(input, null, () => 'key-1');
    const retry = getSubmissionKey(input, first, () => 'key-2');
    expect(retry.key).toBe('key-1');
  });

  it('creates a new key when the payload changes', () => {
    const first = getSubmissionKey({ agent_id: 'agt_1', message: 'hello' }, null, () => 'key-1');
    const changed = getSubmissionKey({ agent_id: 'agt_1', message: 'changed' }, first, () => 'key-2');
