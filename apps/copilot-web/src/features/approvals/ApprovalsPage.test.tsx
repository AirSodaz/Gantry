...
describe("ApprovalsPage", () => {
  ...
  it("shows the pending count, agent, request age, and task link", async () => {
      ...
      items: [
        {
          id: "apr_1",
          task_id: "tsk_1",
          agent_display_name: "Directory agent",
          tool_name: "Directory",
          operation: "update",
          target: "employee/123",
          action_digest: "sha256:exact",
          risk_class: "write",
    ...
    expect(
      await screen.findByText("1 action is waiting for your decision."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Directory agent · employee/123"),
    ).toBeInTheDocument();
