...
function renderDetail() {
  ...
  return render(<QueryClientProvider client={queryClient}><MemoryRouter initialEntries={['/approvals/apr_1']}><Routes><Route path="/approvals/:approvalId" element={<ApprovalDetailPage />} /></Routes></MemoryRouter></QueryClientProvider>);
}
...
describe('ApprovalDetailPage', () => {
  ...
  it('decides from the immutable detail with the exact action digest', async () => {
    ...
    mocked.api.decideApproval.mockResolvedValue({ ...approval, status: 'satisfied', latest_decision: { decision: 'approve', decided_by: 'prn_1', created_at: '2026-08-16T08:00:00Z' } });
  ...
  it('replaces stale detail with the server-winning approval state', async () => {
    mocked.api.getApproval.mockResolvedValue(approval);
    mocked.api.decideApproval.mockRejectedValue(new CopilotApiError(409, 'Approval changed.', 'approval_changed', {
      ...approval,
      status: 'rejected',
      latest_decision: { decision: 'reject', decided_by: 'prn_1', created_at: '2026-08-16T08:00:00Z' },
    }));
    const user = userEvent.setup();
    renderDetail();

    await user.click(await screen.findByRole('button', { name: 'Approve action' }));
  ...
  it('renders server-winning decision evidence instead of decision controls', async () => {
    mocked.api.getApproval.mockResolvedValue({ ...approval, status: 'rejected', latest_decision: { decision: 'reject', reason: 'Choose a different record.', decided_by: 'prn_1', created_at: '2026-08-16T08:00:00Z' } });
    renderDetail();
