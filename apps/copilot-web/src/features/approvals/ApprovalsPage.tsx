...
export function ApprovalsPage() {
  ...
  const query = useInfiniteQuery({
    queryKey: ["approvals"],
    initialPageParam: "",
    queryFn: ({ pageParam }) => api.listApprovals(pageParam),
  ...
  return (
    <div className="page-wrap narrow-page">
      <div className="page-heading">
        <div>
          <span className="eyebrow">Action approvals</span>
          <h1>Review actions before they run.</h1>
          <p>
            {items.length === 1
              ? "1 action is waiting for your decision."
              : `${items.length} actions are waiting for your decision.`}
          </p>
        </div>
      </div>
      {items.length === 0 ? (
          ...
          {items.map((item) => {
            ...
            return (
              <div className="approval-row" key={id}>
                <Link
                  className="approval-row-main"
                  to={`/approvals/${encodeURIComponent(id)}`}
                >
                  <div className="approval-copy">
                    ...
                    <code>{digest}</code>
                    <span>
                      Requested {formatAge(item.created_at)} · Expires{" "}
