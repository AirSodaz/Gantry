...
| Sessions | Sessions, fixed membership, audited owner invitations and Admin self-viewer enrollment, Messages, Runs, requester commands, sequential Run queue, content projections | Authorization decisions, Agent deployment resolver, Attachment binder, scheduler port, event appender | Admin role ownership, approval decisions, runner connection lifecycle, object bytes |
...
| Change Session membership | Idempotency, Session ETag for owner commands or Admin scope check for self-viewer enrollment, Workspace/data-policy checks, membership row, Session event, Audit |
