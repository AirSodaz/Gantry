...
func (s *Service) persistContentSegment(ctx context.Context, runID, streamID string, data []byte) error {
	...
	if err := tx.QueryRow(ctx, `SELECT session_id FROM gantry.runs WHERE id=$1`, runID).Scan(&sessionID); err != nil {
		return err
	}
	messageID, err := sessionmessage.AppendWithID(ctx, tx, sessionID, runID, "agent", sessionmessage.Text(string(data)))
	if err != nil {
		return err
	}
