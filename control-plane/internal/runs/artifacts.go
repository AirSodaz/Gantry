...
func (s *Service) ListSessionArtifacts(ctx context.Context, actor identity.Principal, sessionID string, limit int) ([]Artifact, error) {
	if s.store == nil {
		return []Artifact{}, nil
	}
	rows, err := s.pool.Query(ctx, `SELECT ar.id, ar.session_id, ar.run_id, ar.filename, ar.media_type, ar.size_bytes, ar.digest, ar.classification, ar.scan_status, ar.state, ar.created_at FROM gantry.artifacts ar JOIN gantry.session_members m ON m.session_id=ar.session_id WHERE ar.session_id=$1 AND m.principal_id=$2 ORDER BY ar.created_at, ar.id LIMIT $3`, sessionID, actor.ID, boundedLimit(limit))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// ListMyArtifacts returns artifacts from Sessions the current principal can
// access. It is never a workspace-wide file inventory.
func (s *Service) ListMyArtifacts(ctx context.Context, actor identity.Principal, sessionID, classification, state string, after *ArtifactCursor, limit int) (ArtifactPage, error) {
	if s.store == nil {
	...
	rows, err := s.pool.Query(ctx, `SELECT ar.id, ar.session_id, ar.run_id, ar.filename, ar.media_type, ar.size_bytes, ar.digest, ar.classification, ar.scan_status, ar.state, ar.created_at FROM gantry.artifacts ar JOIN gantry.session_members m ON m.session_id=ar.session_id AND m.principal_id=$1 WHERE ($2='' OR ar.session_id=$2) AND ($3='' OR ar.classification=$3) AND ($4='' OR ar.state=$4) AND ($5::timestamptz IS NULL OR ar.created_at < $5 OR (ar.created_at = $5 AND ar.id < $6)) ORDER BY ar.created_at DESC, ar.id DESC LIMIT $7`, actor.ID, strings.TrimSpace(sessionID), strings.TrimSpace(classification), strings.TrimSpace(state), afterCreatedAt, afterID, pageLimit+1)
	if err != nil {
...
func (s *Service) GetArtifact(ctx context.Context, actor identity.Principal, artifactID string) (Artifact, error) {
	...
	var item Artifact
	err := s.pool.QueryRow(ctx, `SELECT ar.id, ar.session_id, ar.run_id, ar.filename, ar.media_type, ar.size_bytes, ar.digest, ar.classification, ar.scan_status, ar.state, ar.created_at FROM gantry.artifacts ar JOIN gantry.session_members m ON m.session_id=ar.session_id WHERE ar.id=$1 AND m.principal_id=$2`, artifactID, actor.ID).Scan(&item.ID, &item.SessionID, &item.RunID, &item.Filename, &item.MediaType, &item.SizeBytes, &item.Digest, &item.Classification, &item.ScanStatus, &item.State, &item.CreatedAt)
...
func (s *Service) DownloadArtifact(ctx context.Context, actor identity.Principal, artifactID string) (ArtifactDownloadGrant, error) {
	...
	var objectKey, sessionID, runID, workspaceID, classification, state, scanStatus string
	err = tx.QueryRow(ctx, `SELECT ar.object_key, ar.session_id, ar.run_id, t.workspace_id, ar.classification, ar.state, ar.scan_status FROM gantry.artifacts ar JOIN gantry.sessions t ON t.id=ar.session_id JOIN gantry.session_members m ON m.session_id=ar.session_id WHERE ar.id=$1 AND m.principal_id=$2 FOR SHARE OF ar, t, m`, artifactID, actor.ID).Scan(&objectKey, &sessionID, &runID, &workspaceID, &classification, &state, &scanStatus)
	if errors.Is(err, pgx.ErrNoRows) {
		return ArtifactDownloadGrant{}, ErrNotFound
	}
	if err != nil {
...
func (s *Service) DeclareArtifact(ctx context.Context, runnerID string, runID string, epoch uint64, input Artifact) (Artifact, string, time.Time, error) {
	...
	if err := tx.QueryRow(ctx, `SELECT session_id FROM gantry.runs WHERE id=$1 AND runner_id=$2 AND lease_epoch=$3 AND status IN ('accepted','canceling') FOR UPDATE`, runID, runnerID, epoch).Scan(&sessionID); errors.Is(err, pgx.ErrNoRows) {
	...
	objectKey := "artifacts/" + sessionID + "/" + input.ID
	...
	input.SessionID, input.RunID, input.State, input.ScanStatus = sessionID, runID, "declared", "pending"
