...
func (s *Service) Events(ctx context.Context, actor identity.Principal, taskID string, after uint64, limit int) (EventPage, error) {
	...
	err := s.pool.QueryRow(ctx, `SELECT t.id, t.agent_id, a.display_name, t.status, r.id, r.status, r.status_reason, t.created_at, t.task_event_sequence FROM gantry.tasks t JOIN gantry.agents a ON a.id=t.agent_id JOIN gantry.runs r ON r.id=t.current_run_id WHERE t.id=$1 AND t.requester_principal_id=$2`, taskID, actor.ID).Scan(&page.Task.ID, &page.Task.AgentID, &page.Task.AgentDisplayName, &status, &page.Task.CurrentRun.ID, &runStatus, &page.Task.CurrentRun.Reason, &page.Task.CreatedAt, &page.CurrentSeq)
	...
	rows, err := s.pool.Query(ctx, `SELECT e.task_sequence, e.sequence, e.event_type, e.created_at, e.payload FROM gantry.run_events e JOIN gantry.runs r ON r.id=e.run_id WHERE r.task_id=$1 AND e.task_sequence>$2 ORDER BY e.task_sequence LIMIT $3`, taskID, after, limit)
	if err != nil {
		return EventPage{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var event Event
		if err := rows.Scan(&event.Sequence, &event.RunSequence, &event.Type, &event.OccurredAt, &event.Payload); err != nil {
			return EventPage{}, err
		}
